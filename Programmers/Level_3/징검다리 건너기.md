...
def check(stones, k, mid):
    ...
    # stone의 값 0의 의미는 밟을 수 없다는 것
    for s in stones:
        if s - mid <= 0:
            remove_stone += 1
            
            # k만큼 점프할 수 있는데 0인 stone이 연속해서 나온다면 지나갈 수 없음.
            if remove_stone == k:
                return False
        else:
            remove_stone = 0
...
- [코드 및 설명](https://hellominchan.tistory.com/262)
